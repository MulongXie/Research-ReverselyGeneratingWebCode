...
def view_data(start_point, data_position='E:\Mulong\Datasets\dataset_webpage\img_segment'):
    # *** step 1 *** Root Path
    # retrieve global variables for relabel
    global gb_img, gb_label, gb_label_index, gb_newlabelnum
    # set root path
    img_root = os.path.join(data_position, 'img_segment')
    relabel_root = os.path.join(data_position, 'relabel')  # relabels path

    # *** step 2 *** Iterate Web Pages
    # iterate all web pages from the start point
    indices = sorted([int(l[:-4]) for l in os.listdir(img_root)])
    for index in indices[indices.index(start_point):]:
        # set paths
        img_path = os.path.join(img_root, str(index))
        relabel_path = relabel_root, str(index) + '.csv'
