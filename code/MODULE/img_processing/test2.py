...
def grad_to_binary(grad):
    rec, bin = cv2.threshold(grad, 2, 255, cv2.THRESH_BINARY)
    return bin
...
img, gray = read_img('3.png', (0, -1))
grad, binary = preprocess(gray)

cv2.imwrite('processed/grad.png', grad)
cv2.imwrite('processed/bin2.png', binary)
