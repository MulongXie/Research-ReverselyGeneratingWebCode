...
def label_convert(root):
    label_news = ""
    names = os.listdir(os.path.join(root, 'img_segment\\img'))
    for name in names:
        print("name:", name)
        label_path = os.path.join(root, 'img_segment\\label\\' + str(name))
        img_path = os.path.join(root, 'img_segment\\img\\' + str(name))
        ...
        for i in range(len(label_segments)):
            ...
            if seg_no not in label_new:
                label_new[seg_no] = img_path + "\\segment\\" + seg_no + ".png"
