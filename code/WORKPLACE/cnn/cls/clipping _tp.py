...
clipping_root = 'E:\Temp\clipping'


def clipping(org, bboxes, write_path, show=False, padding=(30, 20)):
    clips = []
    for bbox in bboxes:
...
def eval(detection, ground_truth, img_root, show=False):
    def match(org, d_bbox, gt_compos, matched):
        ...
        for i, gt_bbox in enumerate(gt_bboxes):
            ...
            if iou > 0.9 or iod == 1:
                ...
                category_root = pjoin(clipping_root, gt_categories[i])
                if not os.path.exists(category_root):
                    os.mkdir(category_root)

                clipping(org, [d_bbox], pjoin(category_root, str(element_number[gt_categories[i]]) + '.png'), show=show)
                element_number[gt_categories[i]] += 1
                return True
        return False
    ...
    for i, image_id in enumerate(detection):
        ...
        for d_bbox in d_compos['bboxes']:
            match(img, d_bbox, gt_compos, matched)
        if i % 50 == 0:
            print('[%d/%d] %d' %(i, amount, sum(list(element_number.values()))), element_number)
