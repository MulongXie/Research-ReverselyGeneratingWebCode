...
def load_result_single_json(gt_file):
    ...
    compos = {}
    print('Loading %d ground truth' % len(data))
    for annot in tqdm(data):
        if is_bottom_or_top(cvt_bbox(annot['bbox'])):
            continue
        img_name = str(annot['image_id'])
        if img_name not in compos:
            compos[img_name] = {'bboxes': [cvt_bbox(annot['bbox'])], 'categories': [annot['category_id']]}
        else:
...
# detect = load_result_single_json('E:\\Temp\\ocr_results.json')
